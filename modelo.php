...
<?php
	...
	class Formulario{
		...
		function buscar($dato){
			$sql="select * from entrada where Rut like '%".$dato."%' OR Nombre like '%".$dato."%' OR Fechaentrada like '%".$dato."%' OR Horaentrada like '%".$dato."%' OR EntradaSalida like '%".$dato."%' OR observa like '%".$dato."%'";
			$rs=mysqli_query($this->conn, $sql);
			$i=0;
			if(mysqli_num_rows($rs)<1){
			...
			echo "<td align='center'>".$row["Horaentrada"]."</td>";
			echo "<td align='center'>".$row["EntradaSalida"]."</td>";
			echo "<td align='center'>".$row["observa"]."</td>";
			echo '<td align="center">
			<a class="fancybox fancybox.iframe" href="../vista/fancyBoxModificar.php?id='.$row["IDentrada"].'&Rut='.$row["Rut"].'&Nombre='.$row["Nombre"].'&Fechaentrada='.$row["Fechaentrada"].'$Horaentrada='.$row["Horaentrada"].'$observa='.$row["observa"].'" >Editar</a></td>';
			echo "<td><a href='../control/controlador.php?eliminar=si&codigo=".$row["IDentrada"]."'>Eliminar</a></td></tr>";
			$i++;
			}
			}
